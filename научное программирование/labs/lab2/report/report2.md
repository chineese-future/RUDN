...
1) Акканут на Gihub уже в наличии.

2) Настройка глобальных параметров Git.

![Настройка](image1/image1.PNG){ width=70% }

3) Генерация ssh ключа.

![Ssh ключ](image1/image2.PNG){ width=70% }

4) Генерация pgp ключа. И базовая настройка: выбрал RSA and RSA и поставил длину ключа 4096 бит. Выбрал неограниченный срок действия. 

![pgp ключ](image1/image3.PNG){ width=70% }

5) Ввод кодового слова.

![keyword](image1/image4.PNG){ width=70% }

6) Нахождение идентификатора ключа.

![Идентификатор](image1/image5.PNG){ width=70% }

7) Экспорт публичного ключа.

![Экспорт](image1/image6.PNG){ width=70% }

Вторая часть закончена. Мы связали наш репозиторий с репозиторием на github.com.
8) Конвертация отчёта из md с pandoc.
![конвертация] (image1/image7.PNG){ width=70% }
...
Научиться оформлять отчёты с помощью легковесного языка разметки Markdown. Скомпилировал файлы из формата md в pdf и docx через pandoc.
